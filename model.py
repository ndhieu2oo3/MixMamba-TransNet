...
from torch.optim.lr_scheduler import ReduceLROnPlateau, lr_scheduler
...
class SEBlock(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        # Excitation: Two fully connected (FC) layers
    ...
    def forward(self, x):
        # Squeeze step: Global Average Pooling (batch_size, channels, 1, 1)
        se = self.global_avg_pool(x)

        # Excitation step: Fully connected layers
        se = self.fc1(se)  # (batch_size, channels // reduction, 1, 1)
        se = self.relu(se)
        se = self.fc2(se)  # (batch_size, channels, 1, 1)
        se = self.sigmoid(se)  # (batch_size, channels, 1, 1)

        # Scale: Multiply the original input by the recalibrated channel weights
        x = x * se
...
class GatedCNNBlock(nn.Module):
    ...
    """
    def __init__(self, dim, expansion_ratio=8/3, kernel_size=7, conv_ratio=1.0,dilation =1,
        ...
        self.conv1 = nn.Conv2d(conv_channels, conv_channels, kernel_size=3, padding='same', groups=conv_channels)
        self.conv2 = nn.Conv2d(conv_channels, conv_channels, kernel_size=5, padding='same', groups=conv_channels)
        self.conv3 = nn.Conv2d(conv_channels, conv_channels, kernel_size=7, padding='same', groups=conv_channels)
